...
def parse_args():
    ...
    parser.add_argument('-m', '--model_name', type=str, default='stylegan_ffhq',
                        ...
                        help='Name of the model for generation.')
    parser.add_argument('-o', '--output_dir', type=str, default='generated_data',
...
def main():
    ...
    print(MODEL_POOL[args.model_name]['tf_model_path'])
    ...
    for key, val in results.items():
        ...
        if key == 'wp':
            save_path = os.path.join(LATENTS_DIR, f'{key}.npy')
            np.save(save_path, np.concatenate(val, axis=0))
